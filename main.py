...
def api_analyze():
    ...
    - generate_social: Zda generovat odpověď pro sociální sítě (volitelné, výchozí False)
    ...
    try:
        ...
        logger.info(f"Přijat text k analýze: {text[:100]}..." if len(text) > 100 else f"Přijat text k analýze: {text}")
        ...
        if 'image' in request.files and request.files['image'].filename:
            try:
                ...
                logger.info(f"Přijat obrázek k analýze: {image_file.filename}")
                image = Image.open(image_file)
            except Exception as e:
                logger.error(f"Chyba při zpracování nahraného obrázku: {str(e)}")
                return jsonify({"error": f"Chyba při zpracování obrázku: {str(e)}"}), 400
        
        # Získání base64 obrázku
        image_data = request.form.get('image_data', '')
        if not image and image_data:
            try:
                # Odstranění prefixu data URL, pokud existuje
                if 'base64,' in image_data:
                    image_data = image_data.split('base64,')[1]
                
                image_bytes = base64.b64decode(image_data)
                image = Image.open(BytesIO(image_bytes))
                logger.info("Přijat base64 obrázek k analýze")
            except Exception as e:
                logger.error(f"Chyba při zpracování base64 obrázku: {str(e)}")
                return jsonify({"error": f"Chyba při zpracování obrázku: {str(e)}"}), 400
        ...
        # Získání parametru pro generování odpovědi pro sociální sítě
        generate_social = request.form.get('generate_social', 'false').lower() == 'true'
        logger.info(f"Generovat odpověď pro sociální sítě: {generate_social}")
        
        # Analýza obsahu
        try:
            result = analyze_content(text, image, generate_social)
            logger.info(f"Analýza dokončena za {time.time() - start_time:.2f} sekund")
...
def generate_social_response():
    """
    API endpoint pro generování odpovědi pro sociální sítě v různých tónech.
    ...
    Vrací:
    - JSON s vygenerovanou odpovědí
    """
    try:
        analysis = request.form.get('analysis', '')
        tone = request.form.get('tone', 'factual')
