...
5. Identifikovat 3-5 klíčových bodů analýzy a uvést je na začátku vysvětlení
...
Zdroje:
[Seznam zdrojů s odkazy, kategorizovaný na primární a sekundární]"""
        ...
        if generate_social:
            ...
            social_prompt = f"""Na základě této analýzy vytvoř krátkou odpověď v první osobě pro sociální sítě (max 280 znaků),
která shrne hlavní zjištění. Začni s označením "OVĚŘIL/A JSEM" a příslušným emoji (✓, ❓, ✗) podle hodnocení.
...
{response.choices[0].message.content}"""
            ...
            try:
                social_response = openai.chat.completions.create(
                    model="gpt-4o",
                    messages=[
                        {"role": "system", "content": "Jsi stručný a výstižný fact-checker, který odpovídá v první osobě."},
                ...
                result["social"] = "OVĚŘIL/A JSEM ❓ Došlo k chybě při generování odpovědi pro sociální sítě. #FactCheck"
        ...
        if generate_social:
            result["social"] = "OVĚŘIL/A JSEM ❓ Došlo k chybě s API klíčem. Zkontrolujte nastavení. #FactCheck"
        return result
        ...
        if generate_social:
            result["social"] = "OVĚŘIL/A JSEM ❓ Nebyl poskytnut žádný obsah k analýze. #FactCheck"
        return result
        ...
        if generate_social:
            result["social"] = "OVĚŘIL/A JSEM ❓ Došlo k chybě při zpracování obrázku. Zkuste jiný formát. #FactCheck"
        return result
        ...
        if generate_social:
            result["social"] = "OVĚŘIL/A JSEM ❓ Došlo k překročení limitů API. Zkuste to později. #FactCheck"
        return result
        ...
        if generate_social:
            result["social"] = "OVĚŘIL/A JSEM ❓ Došlo k chybě při komunikaci s API. Zkuste to znovu později. #FactCheck"
        return result
        ...
        if generate_social:
            result["social"] = "OVĚŘIL/A JSEM ❓ Došlo k chybě při analyze. Zkuste to prosím znovu později. #FactCheck"
        return result
...
def api_analyze():
    ...
    Očekává data ve formátu multipart/form-data s následujícími parametry:
    - text: Text k analýze (volitelné)
    - image_data: Base64 kódovaná data obrázku (volitelné)
    - generate_social: Zda generovat odpověď pro sociální sítě (volitelné, výchozí hodnota je False)
    
    Vrací JSON s výsledkem analýzy.
    """
    try:
        logger.info("Přijat požadavek na analýzu obsahu")
        ...
        if image_data:
            try:
                logger.info("Zpracování base64 dat obrázku")
                image_bytes = base64.b64decode(image_data)
                image = Image.open(BytesIO(image_bytes))
                logger.info(f"Obrázek úspěšně načten, rozměry: {image.width}x{image.height}, formát: {image.format}")
            except Exception as e:
                logger.error(f"Chyba při zpracování base64 dat obrázku: {str(e)}")
                return jsonify({"error": f"Chyba při zpracování obrázku: {str(e)}"}), 400
        
        # Kontrola, zda byl poskytnut alespoň jeden vstup
        if not text and not image:
            logger.error("Nebyl poskytnut žádný vstup (text ani obrázek)")
            return jsonify({"error": "Musí být poskytnut text nebo obrázek k analýze"}), 400
        
        # Analýza obsahu
        start_time = time.time()
        result = analyze_content(text=text, image=image, generate_social=generate_social)
        end_time = time.time()
        
        logger.info(f"Analýza dokončena za {end_time - start_time:.2f} sekund")
        
        # Zpracování výsledku pro JSON odpověď
        response_data = {
            "analysis": result["analysis"]
        }
        
        # Extrakce hodnocení pravdivosti
        try:
            # Hledání vzoru "Status: X. Název"
            import re
            truth_match = re.search(r"Status:\s*(\d)\.", result["analysis"])
            if truth_match:
                truth_level = int(truth_match.group(1))
                if 1 <= truth_level <= 5:
                    response_data["truth_level"] = truth_level
        except Exception as e:
            logger.warning(f"Chyba při extrakci hodnocení pravdivosti: {str(e)}")
        
        # Extrakce klíčových bodů
        try:
            key_points_section = result["analysis"].split("Klíčové body:")[1].split("Vysvětlení:")[0].strip()
            key_points = [point.strip().lstrip('- ') for point in key_points_section.split('\n') if point.strip()]
            response_data["key_points"] = key_points
        except Exception as e:
            logger.warning(f"Chyba při extrakci klíčových bodů: {str(e)}")
            response_data["key_points"] = []
        
        # Extrakce zdrojů
        try:
            if "Zdroje:" in result["analysis"]:
                sources_section = result["analysis"].split("Zdroje:")[1].strip()
                
                # Pokus o identifikaci primárních a sekundárních zdrojů
                sources = []
                
                if "Primární zdroje:" in sources_section:
                    primary_section = sources_section.split("Primární zdroje:")[1]
                    if "Sekundární zdroje:" in primary_section:
                        primary_section = primary_section.split("Sekundární zdroje:")[0]
                    
                    primary_links = re.findall(r'https?://[^\s\)\]]+', primary_section)
                    for link in primary_links:
                        sources.append({"url": link, "type": "primary"})
                
                if "Sekundární zdroje:" in sources_section:
                    secondary_section = sources_section.split("Sekundární zdroje:")[1]
                    secondary_links = re.findall(r'https?://[^\s\)\]]+', secondary_section)
                    for link in secondary_links:
                        sources.append({"url": link, "type": "secondary"})
                
                # Pokud nebyly nalezeny žádné strukturované zdroje, zkusit najít všechny odkazy
                if not sources:
                    all_links = re.findall(r'https?://[^\s\)\]]+', sources_section)
            ...
            logger.warning(f"Chyba při extrakci zdrojů: {str(e)}")
            response_data["sources"] = []
        
        # Přidání odpovědi pro sociální sítě, pokud byla vygenerována
        if generate_social and "social" in result:
            response_data["social"] = result["social"]
...
@app.route('/api/social-response', methods=['POST'])
@limiter.limit("20 per minute")
def api_social_response():
    """
    API endpoint pro generování odpovědi pro sociální sítě.
    
    Očekává JSON s následujícími parametry:
    - analysis: Text analýzy, ze které se má vygenerovat odpověď
    - tone: Tón odpovědi (factual, sarcastic, aggressive, educational, diplomatic, humorous)
    - length: Délka odpovědi (1=krátká, 2=střední, 3=dlouhá)
    
    Vrací JSON s vygenerovanou odpovědí.
    """
    try:
        logger.info("Přijat požadavek na generování odpovědi pro sociální sítě")
        ...
        if not analysis:
            return jsonify({"error": "Chybí analýza pro generování odpovědi"}), 400
        ...
        # Definice maximální délky podle vybrané hodnoty
        max_length = {1: 140, 2: 280, 3: 500}[length]
        
        # Systémové prompty pro různé tóny
        tone_prompts = {
            'factual': "Jsi fact-checker, který odpovídá v první osobě. Tvůj tón je věcný a neutrální, zaměřený pouze na fakta.",
            'sarcastic': "Jsi fact-checker, který odpovídá v první osobě. Tvůj tón je ironický s jemným humorem, ale stále informativní.",
            'aggressive': "Jsi fact-checker, který odpovídá v první osobě. Tvůj tón je důrazný a nekompromisní při odmítání dezinformací.",
            'educational': "Jsi fact-checker, který odpovídá v první osobě. Tvůj tón je vysvětlující a vzdělávací, poskytující dodatečný kontext.",
            'diplomatic': "Jsi fact-checker, který odpovídá v první osobě. Tvůj tón je vyvážený a diplomatický, respektující různé pohledy.",
            'humorous': "Jsi fact-checker, který odpovídá v první osobě. Tvůj tón je vtipný a lehký, ale stále informativní."
        }
        
        # Výběr správného promptu
        system_prompt = tone_prompts.get(tone, tone_prompts['factual'])
        
        # Příprava uživatelského promptu
        user_prompt = f"""Na základě této analýzy vytvoř krátkou odpověď v první osobě pro sociální sítě (max {max_length} znaků),
která shrne hlavní zjištění. Začni s označením "OVĚŘIL/A JSEM" a příslušným emoji (✓, ❓, ✗) podle hodnocení.
...
{analysis}"""
        ...
        try:
            ...
            social_response = response.choices[0].message.content
            logger.info(f"Úspěšně vygenerována odpověď pro sociální sítě v tónu '{tone}'")
            
            return jsonify({"social": social_response})
        
        except Exception as e:
            logger.error(f"Chyba při generování odpovědi pro sociální sítě: {str(e)}")
            return jsonify({"error": f"Chyba při generování odpovědi: {str(e)}"}), 500
    
    except Exception as e:
        logger.error(f"Neočekávaná chyba v API endpointu pro generování odpovědi: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({"error": f"Neočekávaná chyba: {str(e)}"}), 500
