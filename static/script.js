...
function handleFile(file){
  ...
  dzOverlay.classList.add("hidden");              // <<< schovat ikonu+text
...
function renderResult(data){
  // fallback: pokud backend stále posílá markdown plain text
...
function renderPlain(text){
  // ✔︎ navíc zkusíme vytáhnout markdown [Název](url) do klikacích odkazů
  analysisTbl.classList.add("hidden"); sourcesList.innerHTML="";
  const linkRe=/\\[(.*?)\\]\\((https?:\\/\\/[^\\s)]+)\\)/g;
  const items=[...text.matchAll(linkRe)];
  if(items.length){
    items.forEach(m=>{
      sourcesList.appendChild(sourceLi({title:m[1],url:m[2],relevance:3}));
    });
    sourcesBox.classList.remove("hidden");
  }else sourcesBox.classList.add("hidden");
